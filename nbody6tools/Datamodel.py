...
class Snapshot(object):
    ...
    def __structure(self,record):
        ...
        self.__stars["name"] = record[7][mask]
        self.__stars["mass"] = record[1][mask]
        X = numpy.reshape(record[4],(3,self._ntot),order="F")
        self.__stars["x"] = X[0,:][mask]
        self.__stars["y"] = X[1,:][mask]
        self.__stars["z"] = X[2,:][mask]
        X = numpy.reshape(record[5],(3,self._ntot),order="F")
        self.__stars["vx"] = X[0,:][mask]
        self.__stars["vy"] = X[1,:][mask]
        self.__stars["vz"] = X[2,:][mask]
